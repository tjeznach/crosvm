...
pub fn arch_memory_regions(size: u64) -> Vec<(GuestAddress, u64)> {
    vec![(GuestAddress(RISCV64_PHYS_MEM_START), size)]
}
...
impl arch::LinuxArch for Riscv64 {
        ...
        Vcpu: VcpuRiscv64,
    {
        ...
        let vcpu_init = vec![VcpuInitRiscv64::default(); vcpu_count];
        ...
        Ok(RunnableLinuxVm {
            ...
            bat_control: None,
            pm: None,
            fdt_address: Some(GuestAddress(fdt_offset + RISCV64_PHYS_MEM_START)),
        })
    }
